...
def get_timeseries(subject_list, atlas_name, data_path, silence=False):
    ...
    for i in range(len(subject_list)):
        subject_folder = os.path.join(data_path, f"{subject_list[i]}")  ########### Fatemeh edited
...
def subject_connectivity(
    ...
    if kind in ['tangent', 'correlation', 'partial correlation', 'covariance']:
        ...
        conn_measure = connectome.ConnectivityMeasure(kind=kind, vectorize=vectorize, discard_diagonal=True)
        if kind == 'tangent':
        ...
        conn_measure = connectome.ConnectivityMeasure(kind='tangent', vectorize=vectorize, discard_diagonal=False)
        conn_measure2=conn_measure      
    ...
    # plt.imshow(np.array(input_data[90]), cmap='viridis', interpolation='nearest')
    # plt.colorbar()  # Add a color bar to show the scale
    # plt.title('')
    # plt.xlabel('ROIs')
    # plt.ylabel('ROIs')
    # plt.savefig('./conn_matrix.pdf')
    # # plt.show()

    mask = np.tril(np.ones_like(input_data[90], dtype=bool))
    # Apply the mask to keep only the upper triangle
    upper_triangle_matrix = np.ma.masked_where(mask, input_data[90])

    # Plot the upper triangle of the matrix
    plt.imshow(upper_triangle_matrix, cmap='viridis', interpolation='nearest')
    plt.colorbar()  # Add a color bar to show the scale
    plt.title('Upper Triangle of TP Matrix')
    plt.xlabel('')
    plt.ylabel('')
    plt.savefig('./upper_triangle.pdf')
